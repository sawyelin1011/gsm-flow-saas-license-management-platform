...
import { CreditCard, CheckCircle2, ReceiptText, ArrowUpCircle, Download, ShieldCheck, Zap, Globe, Lock } from 'lucide-react';
import {
  Card,
  CardContent,
  CardHeader,
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <Card className="border-border/50 bg-muted/5 flex flex-col justify-center p-6 space-y-4">
          <div>
            ...
            <p className="text-2xl font-black tracking-tighter">OCT 14, 2025</p>
          </div>
          <Button variant="outline" size="sm" className="h-9 text-[10px] font-black uppercase tracking-widest border-border/50 w-full">Update Payment Method</Button>
        </Card>
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                  ...
                  {isLoadingInvoices ? (
                    ...
                    invoices?.map((inv) => (
                      <TableRow key={inv.id} className="text-xs hover:bg-primary/[0.01]">
                        <TableCell className="font-mono pl-8 text-muted-foreground">{inv.id.toUpperCase()}</TableCell>
        ...
        <div className="space-y-4">
          ...
          <Card className="border-border/50 bg-card p-4 space-y-4 shadow-sm">
          ...
          {upgradePlans.filter(p => p.id !== profile?.planId).map((plan) => (
            <Card key={plan.id} className="p-6 border-border/50 hover:border-primary/50 transition-all flex flex-col justify-between h-40 bg-card/50">
              <div className="flex justify-between items-start">
                ...
                <Badge variant="outline" className="text-[9px] uppercase font-black border-border/50">{plan.limit} Tenants</Badge>
              </div>
              <Button size="sm" className="btn-gradient h-10 text-[10px] font-black uppercase tracking-widest mt-4">Upgrade Authority</Button>
            </Card>
          ))}
        </div>
      </div>
...
function AddOnItem({ icon, title, price }: { icon: React.ReactNode, title: string, price: string }) {
  return (
    <div className="flex items-center justify-between group">
      ...
      <Button variant="ghost" className="h-6 text-[9px] font-black uppercase text-primary hover:bg-primary/10">Add {price}</Button>
    </div>
  );
}